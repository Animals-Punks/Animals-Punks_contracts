...
contract DPunks is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        bytes memory url = abi.encodePacked(baseURI, tokenId.toString());
        bytes memory jsonUrl = abi.encodePacked(url, ".json");
        return bytes(baseURI).length > 0 ? string(jsonUrl) : "";
    }
    ...
    function mintedAll(address to) external onlyOwner {
        require(totalSupply() <= _limitAmount, "ERC721: mint amount is full.");
        for (uint i = 0; i < _limitAmount; i ++ ) {
            _mint(to, i);
        }
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        _afterTokenTransfer(to, tokenId);
    }
